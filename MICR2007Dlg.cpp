// MICR2007Dlg.cpp : implementation file
//
...
BOOL PreMICR2007Print(CDWordArray& dwChangedForms, CFormDisplayDlg* dlg);

void RequeryHistoricalMICR2007Data()
{
	try {
...
BEGIN_MESSAGE_MAP(CMICR2007Dlg, CDialog)
	//{{AFX_MSG_MAP(CMICR2007Dlg)
	ON_WM_SIZE()
	ON_WM_CLOSE()
...
BEGIN_EVENTSINK_MAP(CMICR2007Dlg, CDialog)
    //{{AFX_EVENTSINK_MAP(CMICR2007Dlg)
	//}}AFX_EVENTSINK_MAP
END_EVENTSINK_MAP()

void OnMICR2007Command(WPARAM wParam, LPARAM lParam, CDWordArray& dwChangedForms, int FormControlsID, CFormDisplayDlg* dlg);
void OnMICR2007KeyDown(CDialog* pFormDisplayDlg, MSG* pMsg);

CMICR2007Dlg::CMICR2007Dlg(CWnd* pParent /*=NULL*/)
	: CDialog(CMICR2007Dlg::IDD, pParent)
{
	//{{AFX_DATA_INIT(CMICR2007Dlg)
	...
	m_pOnCommand = &OnMICR2007Command;
	m_pOnKeyDown = &OnMICR2007KeyDown;
	m_pPrePrintFunc = NULL;
	m_ShowWindowOnInit = TRUE;
}

CMICR2007Dlg::~CMICR2007Dlg()
{
	delete m_pframe;
...
void CMICR2007Dlg::DoDataExchange(CDataExchange* pDX)
{
	CDialog::DoDataExchange(pDX);
	//{{AFX_DATA_MAP(CMICR2007Dlg)
...
int CMICR2007Dlg::DoModal(int billID) 
{
	m_ID = billID;
...
/////////////////////////////////////////////////////////////////////////////
// CMICR2007Dlg message handlers

BOOL CMICR2007Dlg::OnInitDialog() 
{
...
//	Template.cdit = 0;
	...
	//m_pPrePrintFunc = &PreMICR2007Print;
...
//	if (!m_pframe->CreateIndirect (&Template, this))//DoModal();
	...
	//Patient Information
	g_aryFormQueries[75].sql.Format("SELECT First + (CASE WHEN Middle <> '' AND Middle <> ' ' THEN ' ' + Left(Middle,1) ELSE '' END)  + ' ' + Last AS PatName, Address1 + ' ' + Address2 AS Address, City + ', ' + State + ' ' + Zip AS CityStateZip, "
	...
	m_pframe->Load (75, where, "", &i); // Lines, Labels, and Patient Information

	where.Format (" WHERE InsuredPartyT.PersonID = %li", m_InsuredPartyID);
	m_pframe->Load (76, where, "", &i); // Primary Insurance Co Info
	...
	if(!bUsePatientAddressASPOS) {
		rs = CreateParamRecordset("SELECT Qualifier, FacilityID FROM InsuranceFacilityID "
		...
		if(!rs->eof) {
			...
			strFacilityQual = AdoFldString(rs, "Qualifier","");
	...
	//load the Box 23 qualifier
	CString strBox23Qual = GetRemotePropertyText("MICR2007_Ref_Order_Qual", "0B", 0, "<None>", true);
	...
	if(bUsePatientAddressASPOS) {
		...
		strPOSFields = "'Patient Home' + ', ' + PatientPersonT.Address1 + CASE WHEN PatientPersonT.Address2 <> '' THEN ', ' + PatientPersonT.Address2 ELSE '' END + ', ' + PatientPersonT.City + ', ' + PatientPersonT.State + ' ' + PatientPersonT.Zip AS POSNameAdd, "
			"PatientPersonT.Zip AS POSZip, '' AS FacilityQual, '' AS FacilityCode, '' AS FacilityNPI, "
		...
		strPOSFields.Format("LocationsT.Name + ', ' + LocationsT.Address1 + CASE WHEN LocationsT.Address2 <> '' THEN ', ' + LocationsT.Address2 ELSE '' END + ', ' + LocationsT.City + ', ' + LocationsT.State + ' ' + LocationsT.Zip AS POSNameAdd, "
			"LocationsT.Zip AS POSZip, '%s' AS FacilityQual, '%s' AS FacilityCode, LocationsT.NPI AS FacilityNPI, "
			"PlaceOfServiceCodesT.PlaceCodes AS ServiceLocation ", _Q(strFacilityQual), _Q(strFacilityID));
	}
	...
	// (a.walling 2014-03-06 14:00) - PLID 61231 - Which codes has changed, using that structure, which also handles the BillDiagCodeT changes
	...
	g_aryFormQueries[77].sql.Format("SELECT BillsT.Date AS BillDate, LineItemT.Date AS ChargeDate, "		
		"COALESCE(( "
			"SELECT TOP 1 "
		...
		"(PersonT.[Last] + ', ' + PersonT.[First] + ' ' + PersonT.Middle + (CASE WHEN (PersonT.Title <> '') THEN (', ' + PersonT.Title) ELSE ('') END)) AS ReferringName, ReferringPhysT.*, CASE WHEN Coalesce(ReferringPhysT.License, '') <> '' THEN '%s' ELSE '' END AS Box23Qual, ReferringPhysT.License AS Box23ID, ReferringPhysT.NPI AS Box23NPI, "
		"%s "
	...
	m_pframe->Load (77, where, " ORDER BY LineID", &i); // Bill/Charge Information

	//load the provider qualifier
	CString strProviderQual;
	strProviderQual.Format("'%s'", _Q(GetRemotePropertyText("MICR2007_Provider_Code_Qual", "1B", 0, "<None>", true)));
	g_aryFormQueries[78].sql.Replace("[ProviderQual]", strProviderQual);

	//load the Box 44 qualifier
	CString strBox44Qual;
	strBox44Qual.Format("'%s'", _Q(GetRemotePropertyText("MICR2007_Ref_Order_Qual", "0B", 0, "<None>", true)));
	g_aryFormQueries[78].sql.Replace("[Box44Qual]", strBox44Qual);
	...
	m_pframe->Load (78, where, "", &i); // Provider Information
	...
	for (unsigned int j = 0; j < m_pframe->m_ControlArray.size(); j++) {
		...
		if (pCtrl->id == 7803 || pCtrl->id == 7808) {
			...
			for (unsigned int k = 0; k < m_pframe->m_ControlArray.size(); k++) {
				...
				if((pCtrl->id == 7803 && (pCtrl2->id == 7804 || pCtrl2->id == 7805 || pCtrl2->id == 7806)) ||
					(pCtrl->id == 7808 && (pCtrl2->id == 7807 || pCtrl2->id == 7809 || pCtrl2->id == 7810))) {
...
void CMICR2007Dlg::OnSize(UINT nType, int cx, int cy) 
{
	CDialog::OnSize(nType, cx, cy);
	SetControlPositions();
}

void CMICR2007Dlg::SetControlPositions(void)
{	
	RECT rect;
...
void CMICR2007Dlg::OnClickX()
{
	EndDialog (0);
}

void CMICR2007Dlg::OnClickCheck() 
{
...
void CMICR2007Dlg::OnClose() 
{
	CDialog::OnClose();
}

BOOL CMICR2007Dlg::OnCommand(WPARAM wParam, LPARAM lParam) 
{
...
void CMICR2007Dlg::OnClickPrint() 
{
	...
	}NxCatchAll("Error printing MIRC 2007 form.");
}

void CMICR2007Dlg::BuildMICRChargesT()
{
	...
	try {
		...
		// (j.gruber 2009-03-18 12:19) - PLID 33574 - discoutns structure
		...
		// (a.walling 2014-03-06 14:00) - PLID 61231 - Which codes has changed, using that structure, which also handles the BillDiagCodeT changes
		// (a.walling 2014-03-06 14:49) - PLID 61231 - Support BillDiagCodeT via BillDiagCodeICD9FlatV/ICD10FlatV aliased as BillDiagCodeFlatV
		...
		g_aryFormQueries[2].sql.Format("SELECT ChargesT.ID, ChargesT.BillID, ChargesT.ItemCode, ChargesT.ItemSubCode, ChargesT.Category, ChargesT.SubCategory, LineItemT.Description, ((CASE WHEN ChargesT.CPTModifier Is Null Then '' ELSE ChargesT.CPTModifier END) + (CASE WHEN ChargesT.CPTModifier2 Is Null Then '' ELSE '  ' + ChargesT.CPTModifier2 END)) AS CPTModifier, "				
			...
			"LEFT JOIN ChargeRespT ON ChargesT.ID = ChargeRespT.ID LEFT JOIN InsuredPartyT ON ChargeRespT.InsuredPartyID = InsuredPartyT.PersonID "
			"OUTER APPLY dbo.%s(BillsT.ID) BillDiagCodeFlatV "
...
void CMICR2007Dlg::OnClickAlign() 
{
...
void CMICR2007Dlg::OnClickCapitalizeOnPrint() 
{
	if(m_CapOnPrint) {
...
void CMICR2007Dlg::OnClickRestore() 
{
	CString strSQL;
	...
	m_pframe->Refresh(75);  // Lines, Labels, and Patient Information
	m_pframe->Refresh(76);  // Primary Insurance Co Info
	m_pframe->Refresh(77);  // Bill/Charge Information
	m_pframe->Refresh(78);  // Provider Information
...
void CMICR2007Dlg::Save(BOOL boSaveAll, CDialog* pdlgWait, int& iPage)
{
...
void CMICR2007Dlg::SaveHistoricalData(BOOL boSaveAll)
{
#ifdef SAVE_HISTORICAL_INFORMATION
...
long CMICR2007Dlg::DoScrollTo(long nNewTopPos)
{
...
void CMICR2007Dlg::OnVScroll(UINT nSBCode, UINT nPos, CScrollBar* pScrollBar) 
{
	switch (nSBCode) {
	case SB_LINEUP:
...
BOOL PreMICR2007Print(CDWordArray& dwChangedForms, CFormDisplayDlg* dlg)
{
	...
	if (nPrintedCharges == 0 && nPrintedProviders == 0) {
		...
		provider_index = -1;
		OnMICR2007Command(IDC_PHYSICIAN_LEFT, 0, dwChangedForms, 0, dlg);
		firstcharge = -6;
		nPrintedProviders++;
	}
	...
	if (nPrintedCharges < nTotalCharges) {		
		OnMICR2007Command(IDC_BILL_DOWN, 0, dwChangedForms, 0, dlg);
		nPrintedCharges += 6;
		return TRUE;
	}
	// Otherwise, go for the next provider
		...
		OnMICR2007Command(IDC_PHYSICIAN_RIGHT, 0, dwChangedForms, 0, dlg);
		firstcharge = -6;
		OnMICR2007Command(IDC_BILL_DOWN, 0, dwChangedForms, 0, dlg);
		nPrintedCharges = 6;
		nPrintedProviders++;
...
void CMICR2007Dlg::OnRadioPaper() {
